...
ConnectionImpl::ConnectionImpl(Event::Dispatcher& dispatcher, ConnectionSocketPtr&& socket,
  ...
  file_event_ = dispatcher_.createFileEvent(
      fd(), [this](uint32_t events) -> void { onFileEvent(events); }, trigger,
...
ConnectionImpl::~ConnectionImpl() {
  ASSERT(SOCKET_INVALID(fd()) && delayed_close_timer_ == nullptr,
...
void ConnectionImpl::closeSocket(ConnectionEvent close_type) {
  ...
  os_syscalls.shutdown(fd(), ENVOY_SHUT_WR);
...
void ConnectionImpl::noDelay(bool enable) {
  ...
  Api::SysCallIntResult result =
      os_sys_calls_.getsockname(fd(), reinterpret_cast<struct sockaddr*>(&addr), &len);
  ...
  result = os_sys_calls_.setsockopt(fd(), IPPROTO_TCP, TCP_NODELAY, &new_value, sizeof(new_value));
...
void ConnectionImpl::onWriteReady() {
  ...
  if (connecting_) {
    ...
    int rc = os_sys_calls_.getsockopt(fd(), SOL_SOCKET, SO_ERROR, &error, &error_size).rc_;
    ASSERT(0 == rc);

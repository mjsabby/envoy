...
ConnectionImpl::ConnectionImpl(Event::Dispatcher& dispatcher, ConnectionSocketPtr&& socket,
    ...
    : ConnectionImplBase(dispatcher, next_global_id_++),
      ...
      os_sys_calls_(Api::OsSysCallsSingleton::get()), read_enabled_(true),
...
void ClientConnectionImpl::connect() {
  ...
  if (result.rc_ == 0) {
    ...
    // "It is normal for EWOULDBLOCK to be reported as the result from
...
#ifdef WIN32
    // On Windows, it is possible for the addressFromFd() to throw an exception

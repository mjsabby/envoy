...
IoHandlePtr InstanceBase::socketFromSocketType(SocketType socket_type) const {
  ...
  const int rc = os_sys_calls_.setsocketblocking(io_handle->fd(), false).rc_;
...
Api::SysCallIntResult PipeInstance::bind(int fd) const {
  ...
  // TODO(Pivotal): Is a cast like this required on Windows?
  //  os_sys_calls_.bind(fd, reinterpret_cast<const sockaddr*>(&address_), sizeof(address_));

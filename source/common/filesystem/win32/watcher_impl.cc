...
WatcherImpl::WatcherImpl(Event::Dispatcher& dispatcher)
  ...
  keep_watching_ = true;
  watch_thread_.reset(new Thread::ThreadImpl([this]() -> void { watchLoop(); }));
}
...
WatcherImpl::~WatcherImpl() {
  ...
  // wait 10s for all outstanding io to be interrupted and for
  // the watcher thread to finsh
  const DWORD wait = ::WaitForSingleObject(watch_thread_->handle(), 10000);

  ASSERT(wait == WAIT_OBJECT_0);
  // on non-debug builds, terminate the thread if it doesn't finish in time
  if (wait != WAIT_OBJECT_0) {
    ENVOY_LOG(error, "failed waiting for thread: {}", wait);
    ::TerminateThread(watch_thread_->handle(), 1);
  }
...
void WatcherImpl::addWatch(const std::string& path, uint32_t events, OnChangedCb cb) {
  ...
  if (!already_watching_dir) {
    ...
    DWORD rc = ::QueueUserAPC(&issueFirstRead, watch_thread_->handle(),

...
InstanceUtil::loadBootstrapConfig(envoy::config::bootstrap::v2::Bootstrap& bootstrap,
  ...
  try {
    if (!options.configPath().empty()) {
      MessageUtil::loadFromFile(options.configPath(), bootstrap, file_system);
    }
  ...
  Json::ObjectSharedPtr config_json =
      Json::Factory::loadFromFile(options.configPath(), file_system);

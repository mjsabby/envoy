...
public:
  GrpcJsonTranscoderConfigTest() : file_system_(Filesystem::fileSystemForTest()) {}
  ...
  std::string makeProtoDescriptor(std::function<void(FileDescriptorSet&)> process) {
    ...
    descriptor_set.ParseFromString(file_system_.fileReadToEnd(
...
TEST_F(GrpcJsonTranscoderConfigTest, ParseBinaryConfig) {
  ...
  proto_config.set_proto_descriptor_bin(
      file_system_.fileReadToEnd(TestEnvironment::runfilesPath("test/proto/bookstore.descriptor")));
  proto_config.add_services("bookstore.Bookstore");
  EXPECT_NO_THROW(JsonTranscoderConfig config(proto_config, file_system_));
}
...
TEST_F(GrpcJsonTranscoderConfigTest, NonBinaryProto) {
  ...
  proto_config.add_services("bookstore.Bookstore");
  EXPECT_THROW_WITH_MESSAGE(JsonTranscoderConfig config(proto_config, file_system_), EnvoyException,
...
public:
  GrpcJsonTranscoderFilterTest(const bool match_incoming_request_route = false)
      : config_(bookstoreProtoConfig(match_incoming_request_route),
...
public:
  GrpcJsonTranscoderFilterPrintTest() {
    ...
    envoy::config::filter::http::transcoder::v2::GrpcJsonTranscoder proto_config{};
    Envoy::Config::FilterJson::translateGrpcJsonTranscoder(*json_config, proto_config);
    config_ = new JsonTranscoderConfig(proto_config, Filesystem::fileSystemForTest());

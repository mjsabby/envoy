...
TEST_P(ProxyProtocolTest, errorRecv_2) {
  ...
  EXPECT_CALL(os_sys_calls, ioctl(_, _, _))
      .Times(AnyNumber())
      .WillRepeatedly(Invoke([](int64_t fd, unsigned long int request, void* argp) {
        const int rc = ::ioctl(fd, request, argp);
        return Api::SysCallIntResult{rc, errno};
      }));
  EXPECT_CALL(os_sys_calls, writev(_, _, _))
      .Times(AnyNumber())
      .WillRepeatedly(Invoke([](int64_t fd, const struct iovec* iov, int iovcnt) {
  ...
  EXPECT_CALL(os_sys_calls, close(_)).Times(AnyNumber()).WillRepeatedly(Invoke([](int64_t fd) {
...
TEST_P(ProxyProtocolTest, v2ParseExtensionsIoctlError) {
  ...
  EXPECT_CALL(os_sys_calls, recv(_, _, _, _))
      .Times(AnyNumber())
      .WillRepeatedly(Invoke([](int64_t fd, void* buf, size_t len, int flags) {

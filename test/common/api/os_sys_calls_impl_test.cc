...
class OsSysCallsImplTest : public TestBaseWithParam<Network::Address::IpVersion> {};
...
TEST_P(OsSysCallsImplTest, SocketPair) {
  ...
  ASSERT_EQ(r1.rc_, 0);
  ASSERT_TRUE(SOCKET_VALID(socks[0]));
  ASSERT_TRUE(SOCKET_VALID(socks[1]));

  ScopedSocketCloser sock0(socks[0]);
  ScopedSocketCloser sock1(socks[1]);

  sockaddr_storage sa;
  socklen_t sa_len = sizeof(sa);
  r1 = os_sys_calls.getsockname(socks[0], reinterpret_cast<sockaddr*>(&sa), &sa_len);
  ASSERT_EQ(r1.rc_, 0);
  ASSERT_EQ(sa.ss_family, family);

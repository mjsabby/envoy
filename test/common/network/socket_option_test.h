...
public:
  SocketOptionTest() {
    ...
    EXPECT_CALL(os_sys_calls_, setsocketblocking(_, _))
        .Times(AnyNumber())
        .WillRepeatedly(Invoke([this](SOCKET_FD sockfd, bool block) -> Api::SysCallIntResult {
  ...
  void testSetSocketOptionSuccess(
    ...
    for (auto state : when) {
      if (option_name.has_value()) {
        ...
        // TODO: Pivotal - if there are problems with this test we might want to switch to the
        // version below
#ifdef WIN32_ALTERNATE_FLAVOR

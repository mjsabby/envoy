...
function bazel_test($type, $test) {
  if ($test) {
    ...
    bazel $env:BAZEL_BASE_OPTIONS.Split(" ") test $env:BAZEL_BUILD_OPTIONS.Split(" ") -c $type "//test/..." --test_tag_filters=-skip_on_windows --build_tests_only --test_summary=terse --test_output=errors -- "-//test/extensions/..." "-//test/integration/..." "-//test/tools/type_whisperer/..."
  }
  exit $LASTEXITCODE
}
